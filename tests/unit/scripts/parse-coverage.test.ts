...
describe('parse-coverage.js helper functions', () => {
  ...
  describe('generatePRComment', () => {
    ...
    it('should include table with all metrics and thresholds', () => {
      const comment = generatePRComment(mockCoverage);

      expect(comment).toContain('| Metric | Coverage | Threshold | Status |');
    ...
    it('should include legend', () => {
      ...
      expect(comment).toContain('### Legend');
      expect(comment).toContain('✅ Passing | ❌ Failing');
    });
    ...
    it('should show thresholds from vitest.config.ts', () => {
      const comment = generatePRComment(mockCoverage);

      // Should show threshold percentages if vitest.config.ts exists
      // Otherwise shows '-' for missing thresholds
      expect(comment).toMatch(/\| Statements \|.*\| (?:\d+%|-) \|/);
      expect(comment).toMatch(/\| Branches \|.*\| (?:\d+%|-) \|/);
    });

    it('should show threshold warning when coverage is below threshold', () => {
      const lowCoverage = {
        statements: { pct: 45.5, covered: 5000, total: 11000 },
      ...
      // Should contain warning if thresholds exist and are not met
      expect(comment).toContain('| Statements | 45.5% (5000/11000) |');
      expect(comment).toContain('| Branches | 50% (1000/2000) |');
      expect(comment).toContain('| Functions | 55% (300/545) |');
    });
    ...
    it('should handle perfect coverage', () => {
      ...
      expect(comment).toContain('| Statements | 100% (10000/10000) |');
      expect(comment).toContain('| Branches | 100% (2000/2000) |');
      expect(comment).toContain('| Functions | 100% (500/500) |');
      expect(comment).toContain('| Lines | 100% (10000/10000) |');
    });
  });

  describe('checkCoverageThresholds', () => {
    const { checkCoverageThresholds } = require('../../../scripts/parse-coverage.js');

    it('should return true when all coverage meets thresholds', () => {
      const goodCoverage = {
        statements: { pct: 80, covered: 8000, total: 10000 },
        branches: { pct: 80, covered: 1600, total: 2000 },
        functions: { pct: 80, covered: 400, total: 500 },
        lines: { pct: 80, covered: 8000, total: 10000 }
      };

      const result = checkCoverageThresholds(goodCoverage);
      expect(result).toBe(true);
    });

    it('should return false when any coverage is below threshold', () => {
      const lowCoverage = {
        statements: { pct: 45.5, covered: 5000, total: 11000 },
        branches: { pct: 50, covered: 1000, total: 2000 },
        functions: { pct: 55, covered: 300, total: 545 },
      ...
      const result = checkCoverageThresholds(lowCoverage);
      expect(result).toBe(false);
    });

    it('should handle exact threshold values', () => {
      const exactCoverage = {
        statements: { pct: 75, covered: 7500, total: 10000 },
        branches: { pct: 75, covered: 1500, total: 2000 },
        functions: { pct: 70, covered: 350, total: 500 },
        lines: { pct: 75, covered: 7500, total: 10000 }
      };

      const result = checkCoverageThresholds(exactCoverage);
      expect(result).toBe(true);
    });
  });
});
